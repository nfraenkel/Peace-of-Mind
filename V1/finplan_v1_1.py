...
# ------------------
def iterate_withdrawal(initialStep, stepEpsilon, finalDelta, targetEndValue, finPlan):    
    '''
    ...
    '''
    withdrawal = 0  # EndFunds will be large and deltaFunds will be negative
    keepGoing = True
    step = initialStep
    lastDelta = 1000000000  # to ensure that we will iterate at least a couple of times
    
    while keepGoing:
        EndFunds = run_all_years(withdrawal, finPlan)
        deltaFunds = targetEndValue - EndFunds
        deltaString = locale.format("%d", int(deltaFunds), grouping=True)
        withd_str = locale.format("%d", int(withdrawal), grouping=True)
        print ('Pre-return rate = %.2f%% Retirement-return rate = %.2f%% withdrawal = $%s  end funds = %.2f  delta = %s step = %.2f' % 
        (finPlan['PreReturnRate'], finPlan['RetirementReturnRate'], withd_str, EndFunds, deltaString, step))

        if (same_sign(deltaFunds,step)): # check if it's time to adjust the step
            if (math.fabs(deltaFunds) > math.fabs(lastDelta)): # check if previous iteration was closer
                # The previous iteration gave better results, keep lastEndFunds and lastDelta
                step = stepEpsilon * step # continue iterating in same direction, with smaller step
                withdrawal -= step
                # print ('\t NEW step = %d' % (step))
            else: # this iteration was closer
                lastEndFunds = EndFunds
                lastDelta = deltaFunds
                # Change direction of iteration and step size
                step = - stepEpsilon * step
                withdrawal += step
                # print ('\t NEW step = %d' % (step))
            if (math.fabs(deltaFunds) <= finalDelta and (EndFunds > 0)): # we're done
                # print ('Done: final step = %.2f' % step)
                keepGoing = False
                break
        else: # Continue stepping in the same direction            
            lastEndFunds = EndFunds
            lastDelta = deltaFunds
            withdrawal += step
...
def RunScenario(finPlan):
    ...
    withdrawal, EndFunds = iterate_withdrawal(InitialStep, StepEpsilon, FinalDelta, DefaultEndFunds, finPlan)
    
    finPlan['Withdrawal'] = withdrawal # to show we did not compute anything
