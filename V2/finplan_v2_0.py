...
V2 Use MonteCarlo simulation - and compute the withdrawal amounts based on individual Return Rates for each year
'''
...
DefaultSuccessRatio = 80 # % between 0-100
...
# ------------------
def run_all_years_v2(WithdrawalAmount, finPlan, rateArray):
    ...
    retirementReturnRate = float(finPlan['RetirementReturnRate']) * 0.01
    for yr in range(0,NbYears):
        returnRate = rateArray[yr] * 0.01
        inflationWithdrawal *= (1+inflationRate)
        if (yr < withdrawalStart):  # Before retirement
            withdrawal = - finPlan['PreContribution']  # Positive contribution == negative withdrawal
        else: # During retirement
            withdrawal = inflationWithdrawal
        EndFunds = run_1_year(StartFunds, returnRate, withdrawal)
            
        s_f = locale.format("%d", int(StartFunds), grouping=True)
        e_f = locale.format("%d", int(EndFunds), grouping=True)
        # print ('Year: %d: start = $%s - end = $%s - rate = %.2f%%  withdrawal = $%.2f' % (yr,s_f, e_f,returnRate, withdrawal))
        StartFunds = EndFunds
...
# ------------------
def MonteCarlo(finPlan, MeanRate, StddevRate,NbRun,SuccessRatio):
    '''
    For the time being, only the retirement period
    For each of NbRun, compute an array of Return Rates for each year, based on the MeanRate and StddevRate normal distribution
    ...
    # ToDo: break up the rate computation Pre vs Retirement
    NbYears = finPlan['AgeEnd'] - finPlan['AgeToday']
    withdrawalList =[]
    rateList =[]
    run = 0
    while (run < NbRun):
        ...
        rateArray = np.random.normal(MeanRate, StddevRate, size = NbYears)
    ...
    withdrawalResult = findSuccess(withdrawalList, SuccessRatio)
    print('Result = ${:,} - min = ${:,} - max = ${:,}'.format(int(withdrawalResult), int(minWithdrawal),int(maxWithdrawal)))
    ...
    print ('Requested/Actual: mean = %f / %f  stddev = %f / %f' % (MeanRate, mean, StddevRate, stddev))
    
    finPlan['Withdrawal'] = withdrawalResult # to show the result
    finPlan['HasResult'] = True
    return(finPlan)    
...
class SinglePlanAPI(Resource):
    ...
    def post(self, plan_id):
        ...
        if (args.get('Run', None) != True):
                ...
                abort(500)        
        # RunScenario(finplan)
        finplan = MonteCarlo(finplan, DefaultMeanRate, DefaultStddevRate, DefaultNbRun, DefaultSuccessRatio)
